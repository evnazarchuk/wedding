...
class Recording extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('product_id, date, time, user_name, phone', 'required'),
			array('phone', 'numerical', 'integerOnly'=>true),
			array('product_id', 'length', 'max'=>10),
			array('user_name, email', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, product_id, date, time, user_name, email, phone, comment', 'safe', 'on'=>'search'),
		);
	}
